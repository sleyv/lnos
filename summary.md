...
- **setup.sh** — автоопределение пакетного менеджера (apt/pacman/dnf/zypper/emerge/apk), сборка, 3 варианта выбора имени (hostname/random/manual) с проверкой коллизий через `lnosctl resolve`. Работает без `sudo`: привилегированные шаги запрашивают sudo только в момент. Если `lnosd` уже запущен — предупреждает и предлагает продолжить.
  - *Исправлено интерактивное перенаправление TTY* при пайпинге из curl (удалены скобки подоболочки `(exec < /dev/tty)`).
  - *Исправлены права доступа* на создание конфигурации и ключей в `/etc/lnos` (используется `$SUDO` вместо `$AS_USER`).
  - *Исправлен запуск и остановка временного демона* при проверке коллизий (переведён на `$SUDO` с безопасным пуллингом при завершении процесса).
  - *Исправлена инициализация `owners.db`*: теперь туда записывается только корректный суффикс владельца хоста (`NODE_OWNER="${NODE_NAME##*.}"`) через `$SUDO tee`, что решает проблему с резолвом NSS.
  - *Обновлён ASCII-логотип* на чистый, красивый и симметричный.
  - *Исправлено падение под `set -e` при свободном имени*: добавлена защита `|| true` к вызову `lnosctl resolve` в `check_name_free`, чтобы предотвратить принудительный выход из скрипта, когда имя свободно.
