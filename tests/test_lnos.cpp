...
TEST(LnosProtocolTest, GossipResponseEncodeDecode) {
    lnos::Packet original;
    original.type = lnos::PacketType::GossipResponse;

    lnos::GossipNode node;
    node.name = "response.node";
    node.ip = "10.0.0.5";
    node.publicKey.fill(0xDD);
    original.gossipNodes.push_back(node);
    original.publicKey.fill(0xEE);
    original.signature.fill(0xFF);

    lnos::Blob blob = lnos::encode(original, true);
    lnos::EncodedPacket encoded{blob.data(), blob.size()};
    lnos::Packet decoded;
    ASSERT_TRUE(lnos::decode(encoded, decoded));
    EXPECT_EQ(decoded.type, lnos::PacketType::GossipResponse);
    ASSERT_EQ(decoded.gossipNodes.size(), 1);
    EXPECT_EQ(decoded.gossipNodes[0].name, "response.node");
    EXPECT_EQ(decoded.gossipNodes[0].ip, "10.0.0.5");
}

TEST(LnosProtocolTest, GossipNodeMaxServices) {
    lnos::Packet original;
    original.type = lnos::PacketType::GossipRequest;

    lnos::GossipNode node;
    node.name = "maxsvc.gossip";
    node.ip = "10.0.0.99";
    for (int i = 0; i < 256; ++i) {
        node.services.push_back({"s" + std::to_string(i), static_cast<uint16_t>(i)});
    }
    node.publicKey.fill(0x12);
    original.gossipNodes.push_back(node);
    original.publicKey.fill(0x34);
    ...
    ASSERT_TRUE(lnos::decode(encoded, decoded));
    ASSERT_EQ(decoded.gossipNodes.size(), 1);
    ASSERT_EQ(decoded.gossipNodes[0].services.size(), 256);
    EXPECT_EQ(decoded.gossipNodes[0].services[0].name, "s0");
    EXPECT_EQ(decoded.gossipNodes[0].services[255].name, "s255");
}

TEST(LnosProtocolTest, GossipNodeWithEmptyName) {
    lnos::Packet original;
    original.type = lnos::PacketType::GossipRequest;
    ...
    node.name = "";
    node.ip = "::1";
    node.publicKey.fill(0xAA);
    original.gossipNodes.push_back(node);
    original.publicKey.fill(0xBB);
    original.signature.fill(0xCC);
    ...
    ASSERT_TRUE(lnos::decode(encoded, decoded));
    ASSERT_EQ(decoded.gossipNodes.size(), 1);
    EXPECT_EQ(decoded.gossipNodes[0].name, "");
    EXPECT_EQ(decoded.gossipNodes[0].ip, "::1");
}

TEST(LnosProtocolTest, AnnouncePacketNoServices) {
    lnos::Packet original("bare.node", {});
    original.publicKey.fill(0);
    original.signature.fill(0);
    ...
    lnos::Packet decoded;
    ASSERT_TRUE(lnos::decode(encoded, decoded));
    EXPECT_EQ(decoded.announce.name, "bare.node");
    EXPECT_TRUE(decoded.announce.services.empty());
}

TEST(LnosProtocolTest, EncodeDecodeEncryptedAnnounceWithIsEncrypted1And2) {
    // isEncrypted=1 (multicast) and isEncrypted=2 (unicast) both
    // use the same encrypted packet structure: nonce + payload length + payload
    for (uint8_t encVal : {1, 2}) {
        lnos::Packet p("enc.node", {{"svc", 42}});
        p.isEncrypted = encVal;
        p.nonce.fill(0xAB);
        p.encryptedPayload = {0x01, 0x02, 0x03, 0x04, 0x05};
        p.publicKey.fill(0xCD);
        p.signature.fill(0xEF);

        lnos::Blob blob = lnos::encode(p, true);
        lnos::EncodedPacket encoded{blob.data(), blob.size()};
        lnos::Packet decoded;
        ASSERT_TRUE(lnos::decode(encoded, decoded));
        EXPECT_EQ(decoded.isEncrypted, encVal);
        EXPECT_EQ(decoded.nonce, p.nonce);
        EXPECT_EQ(decoded.encryptedPayload.size(), 5);
        ASSERT_GE(decoded.encryptedPayload.size(), 5);
        EXPECT_EQ(decoded.encryptedPayload[0], 0x01);
        EXPECT_EQ(decoded.encryptedPayload[4], 0x05);
        EXPECT_TRUE(decoded.announce.name.empty());
        EXPECT_TRUE(decoded.announce.services.empty());
    }
}

TEST(LnosProtocolTest, EncodeDecodeEmptyEncryptedPayload) {
    lnos::Packet p("", {});
    p.isEncrypted = 1;
    p.nonce.fill(0);
    p.publicKey.fill(0);
    ...
    ASSERT_TRUE(lnos::decode(encoded, decoded));
    EXPECT_EQ(decoded.isEncrypted, 1);
    EXPECT_TRUE(decoded.encryptedPayload.empty());
}

TEST(LnosProtocolTest, DecryptAlreadyDecryptedPacketIsIdempotent) {
    ASSERT_GE(sodium_init(), 0);
    ...
    std::memcpy(pk.data(), pub, PUBLIC_KEY_SIZE);
    std::array<uint8_t, PRIVATE_KEY_SIZE> sk;
    std::memcpy(sk.data(), priv, PRIVATE_KEY_SIZE);

    lnos::Packet p("plain.node", {{"svc", 1}});
    p.type = lnos::PacketType::Announce;
    p.publicKey = pk;
    ASSERT_TRUE(lnos::signPacket(p, sk));

    // isEncrypted = 0 — decrypt is a no-op (returns true)
    EXPECT_TRUE(lnos::decryptPacketPayload(p, sk, pk, true));

    // encrypt it
    ASSERT_TRUE(lnos::encryptPacketPayload(p, sk, pk, true));
    EXPECT_EQ(p.isEncrypted, 1);

    // decrypt once
    ASSERT_TRUE(lnos::decryptPacketPayload(p, sk, pk, true));
    EXPECT_EQ(p.isEncrypted, 0);
    EXPECT_EQ(p.announce.name, "plain.node");

    // decrypt again — idempotent, still returns true
    EXPECT_TRUE(lnos::decryptPacketPayload(p, sk, pk, true));
    EXPECT_EQ(p.isEncrypted, 0);
    EXPECT_EQ(p.announce.name, "plain.node");
}

TEST(LnosProtocolTest, SignPacketChangesSignature) {
    ASSERT_GE(sodium_init(), 0);
    ...
    lnos::Packet p("sign.test", {});
    p.publicKey = pk;

    std::array<uint8_t, SIGNATURE_SIZE> before = p.signature;
    ASSERT_TRUE(lnos::signPacket(p, sk));
    std::array<uint8_t, SIGNATURE_SIZE> after = p.signature;

    EXPECT_NE(before, after);
    EXPECT_TRUE(lnos::verifyPacket(p));
}

TEST(LnosProtocolTest, EncodeDecodeGossipNodeWithIpv4MappedIpv6) {
    lnos::Packet original;
    original.type = lnos::PacketType::GossipRequest;

    lnos::GossipNode node;
    node.name = "ipv4mapped";
    node.ip = "::ffff:192.168.1.1";
    node.publicKey.fill(0x42);
    original.gossipNodes.push_back(node);
    original.publicKey.fill(0x24);
    original.signature.fill(0x00);
    ...
    ASSERT_TRUE(lnos::decode(encoded, decoded));
    ASSERT_EQ(decoded.gossipNodes.size(), 1);
    EXPECT_EQ(decoded.gossipNodes[0].ip, "::ffff:192.168.1.1");
}

TEST(LnosProtocolTest, MultipleGossipNodesEncodeDecode) {
    lnos::Packet original;
    original.type = lnos::PacketType::GossipResponse;

    for (int i = 0; i < 5; ++i) {
        lnos::GossipNode node;
        node.name = "gossip" + std::to_string(i);
        node.ip = "10.0.0." + std::to_string(i + 1);
        node.services = {{"svc" + std::to_string(i), static_cast<uint16_t>(1000 + i)}};
        node.publicKey.fill(static_cast<uint8_t>(0x10 + i));
        original.gossipNodes.push_back(node);
    }
    original.publicKey.fill(0x99);
    original.signature.fill(0x88);

    lnos::Blob blob = lnos::encode(original, true);
    lnos::EncodedPacket encoded{blob.data(), blob.size()};
    lnos::Packet decoded;
    ASSERT_TRUE(lnos::decode(encoded, decoded));
    ASSERT_EQ(decoded.gossipNodes.size(), 5);
    EXPECT_EQ(decoded.gossipNodes[0].name, "gossip0");
    EXPECT_EQ(decoded.gossipNodes[4].name, "gossip4");
    EXPECT_EQ(decoded.gossipNodes[0].ip, "10.0.0.1");
    EXPECT_EQ(decoded.gossipNodes[4].ip, "10.0.0.5");
    EXPECT_EQ(decoded.gossipNodes[0].services[0].port, 1000);
    EXPECT_EQ(decoded.gossipNodes[4].services[0].port, 1004);
}

TEST(LnosProtocolTest, DecodeUnknownPacketTypeReturnsFalse) {
    // type 99 is not in PacketType enum
    lnos::Blob custom;
    lnos::blobPush(custom, std::string("3"));
    lnos::blobPush(custom, (uint16_t)99);
    custom.push_back(0); // isEncrypted = 0
    lnos::blobPush(custom, std::string("x"));
    lnos::blobPush(custom, (uint64_t)0);
    std::array<uint8_t, PUBLIC_KEY_SIZE> pub{};
    std::array<uint8_t, SIGNATURE_SIZE> sig{};
